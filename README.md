# FlyRabbit 集成mysqldump、mysqlbinlog、xtrabackup等开源工具来完成MySQL的备份、恢复任务
# 工具简介
只需要根据备份场景，在策略配置文件中配置策略；添加需要备份的实例，即可自动完成备份、恢复、巡检、归档等需要DBA手动完成的任务。FlyRabbit主要是调用第三方的工具来自动完成备份、巡检、恢复等任务。将策略与备份逻辑解耦，全程不需要额外编写脚本。目前计划实现的功能如下：
 -  执行备份操作
...
# 使用pip进行安装
 pip install flyrabbit-1.0.0-.py3-none-any.whl
``` 
FlyRabbit 将备份策略，备份任务、备份实例以及巡检记录存储到MySQL数据库中。因此，在安装好FlyRabbit之后，需要执行下列语句来初始化mydbs
```mysql-sql
mysql -u root -p < flyrabbit.sql
```
